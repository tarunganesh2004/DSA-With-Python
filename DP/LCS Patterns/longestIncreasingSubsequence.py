...
def bruteForce(arr): # O(n^2)
    ...
    def recursion(idx,prev): # here cur_idx,prev_idx, instead of prev_idx we can use prev_val alsoo
        if idx==len(arr):
            return 0
        # not take
...
def tabulation(arr):
    ...
    for idx in range(n-1,-1,-1):
        ...
        for prev in range(idx-1,-2,-1): # prev can have -1 to idx-1, -2 bcz it excludes this value
